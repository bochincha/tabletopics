...
with open(csvfile, 'rb') as csvfile:
	...
	for row,line in enumerate(dialogs):
		...
		if len(line[2]) and len(line[3]):
			...
			code = int(line[3])
			# Save this dialog
			query = "INSERT INTO opinion (dialog_id, opt, tag, code) VALUES(%d, 1, '%s', %d)"%(dialog_id, tag, code)
			#print query
			cur.execute(query)
			db.commit()
		...
		if len(line[4]) and line[5]:
			...
			code = int(line[5])
			# Save this dialog
			query = "INSERT INTO opinion (dialog_id, opt, tag, code) VALUES(%d, 2, '%s', %d)"%(dialog_id, tag, code)
			cur.execute(query)
			db.commit()
